...
本地执行层已经抽出 `AgentRunner` 接口。当前默认实现是 Codex CLI 的 `exec` runner；后续可以在不改 Pipeline 状态机的前提下切换到 opencode、Claude Code 或长期 session runner。
...
- Agent runtime 文件记录当前 repo target、workspace root、runner、sandbox policy。
