...
- Release 打包后，Electron 读取 `Resources/web/index.html` 并启动 `Resources/bin/omega-local-runtime`；CLI 二进制 `omega` 同时随桌面包放入 `Resources/bin`，也会输出到 `dist/release/bin/omega` 供命令行演示。
...
- Full workspace snapshot 只作为兼容、导出和灾难恢复层；UI session、详情页轮询、JobSupervisor idempotency 不应重新依赖反序列化整份 snapshot。数据归属边界见 `docs/data-model.md`。
...
- Agent Profile 的 stage Skills / MCP 会在 runner 启动前物化到运行 workspace 的 `.omega/agent-capabilities.json`、`.omega/agent-capabilities.md`、`.omega/agent-skill-manifest.json`、`.omega/skills/<skill>/SKILL.md`、`.codex/OMEGA.md` 和 `.claude/CLAUDE.md`，并注入 runner 子进程环境变量。Skills 优先复制本机安装的 `SKILL.md`，缺失时生成项目 fallback，不再只是 UI 字段或 prompt 文本。
...
- DevFlow 的角色型阶段会消费 Agent Profile runner 配置：Requirement 产出需求 handoff，Master 产出 dispatch / rework assessment / rework checklist，Architect 是默认 Plan 等价阶段并产出 `technical-plan`、`functional-todo-list`、`project-todo-list`，Testing 在本地验证命令之后由 Testing Agent 复核并写最终 test report，Delivery 在 human gate 前产出 handoff。runtime 仍负责 git/GitHub/checkpoint/merge 等确定性动作，职责边界见 `docs/devflow-agent-execution-policy.md`。
...
## CLI 与 Desktop Release

- CLI 入口位于 `services/local-runtime/cmd/omega`，实现位于 `services/local-runtime/internal/omegacli`。
- CLI 是 runtime API 的薄封装，不直接读写 SQLite，也不绕过 checkpoint decision、DevFlow run、JobSupervisor tick、proof preview 等后端路径。
- 常用列表命令默认使用 compact / filtered read model，避免重新拉取 full workspace snapshot。
- `scripts/build-release-binaries.mjs` 构建 `omega-local-runtime` 和 `omega` 到 `dist/release/bin`。
- `electron-builder` 输出 macOS `.dmg` / `.zip` 到 `dist/desktop`，product name 为 `Omega`。
- 当前本机演示包采用 ad-hoc / unsigned 路径构建；公开发布需要 Developer ID 签名与 notarization，详见 `docs/release-packaging.md`。
