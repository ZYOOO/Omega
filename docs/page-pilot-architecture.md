...
Page Pilot 当前主模式是 Electron direct pilot：
...
- 用户应在 Omega 的 Page Pilot preview surface 中打开“正在构建的软件”的页面。
- Overlay / preload 绑定到目标项目 preview document，再圈选目标项目里的按钮、标题、卡片文案。
- 浏览器 SPA 模式只能直接 inspect same-origin iframe；Electron BrowserView + preload bridge 是功能二的主路径，用于真实目标项目预览圈选。
- 改完代码后，Go runtime 负责写源码 / 后续重启目标 dev server，Electron 负责刷新内部预览页面。

Overlay 能力：

- 以悬浮按钮注入 Page Pilot preview surface。
- 点击 `Select` 后进入元素圈选模式。
...
- 打开 Overlay 时读取最近 Page Pilot runs，方便继续确认或查看已撤销 / 已交付记录。

Electron dev shell 能力：

- `BrowserWindow` 加载 Omega React SPA。
- `BrowserView` 加载用户目标项目 preview URL。
- `preview-preload.cjs` 注入最小 DOM selection bridge，并把 selector/context/source metadata 回传给 Omega。
- `omega-preload.cjs` 暴露 `omegaDesktop.openPreview`、`omegaDesktop.reloadPreview` 和 selection event 订阅。

Electron direct pilot mode：

- `npm run desktop:pilot` 直接打开用户目标项目 URL，默认 `http://127.0.0.1:5173/`。
- `pilot-preload.cjs` 在目标项目页面内注入 Page Pilot 悬浮手指按钮、hover 高亮、元素信息 tooltip 和修改输入框。
...
4. 对 GitHub target，只有当前 local runtime 工作目录的 git remote 与 target 匹配时才使用当前 worktree，保证 Vite HMR 能看到源码变更。
5. 校验 `sourceMapping.file` 不逃逸 repository root，且文件真实存在。
6. 读取 Project / Repository Agent Profile，按 `coding` Agent 解析 runner。
7. 对 Codex / opencode / Claude Code 做 CLI preflight，缺失时在启动前失败。
8. 运行 Agent 修改真实 source file。Prompt 明确声明传入的 selection 是 primary target，多批注内容只作为辅助上下文。
9. 返回 changed files、diff summary、line-level diff summary 和 preview refresh 提示。
10. 将 run 记录持久化到本地 runtime，返回 `runId`，供确认交付或撤销使用。
...
- GitHub target 不能随意 clone 到临时目录做“看不见”的修改；为了 HMR，MVP 只接受匹配当前 runtime worktree 的 GitHub repo。
...
- direct pilot apply 成功后自动刷新 Electron 目标页面；必要时先调用 runtime 的 target dev server restart API。
- 增加 Preview Runtime Agent/Profile：自动理解不同项目的 install/start/health/reload 方式。
- 增加 preview runtime process supervisor：持久化 pid、stdout/stderr、端口、健康检查和失败诊断。
- 在 direct pilot 中补齐 patch preview / diff 摘要 / 用户确认 / deliver / discard 的完整浮层 UI。
- 将 direct pilot 的 target URL、project id、repository target id、local worktree 绑定成一等配置，替代 env/default。
- 支持同一 Page Pilot run 的多轮对话：用户看到修改后继续追加批注或补充要求，Agent 复用同一上下文迭代。
- 增加 DOM-only source locating 辅助能力：当缺少 `data-omega-source` 时，用 selector、文本、样式和 repository 搜索生成候选源码位置，但仍以显式 metadata 为优先。
- 增加 Go target dev server start/restart API，支持 `npm run dev` 等命令。
...
- 在 PR body 中加入更结构化的 DOM/context 摘要和截图证据。
