...
- Stage 级 Skills / MCP 不是单纯 UI 配置。runner 启动前会写入 `.omega/agent-capabilities.json`、`.omega/agent-capabilities.md`、`.omega/agent-skill-manifest.json`、`.omega/skills/<skill>/SKILL.md`、`.codex/OMEGA.md` 和 `.claude/CLAUDE.md`，并注入 `OMEGA_AGENT_SKILLS` / `OMEGA_AGENT_SKILL_ROOT` / `OMEGA_AGENT_SKILL_PATHS` / `OMEGA_AGENT_MCP` 环境变量；默认映射、本机安装复制和项目 fallback 行为见 `docs/agent-skills-and-mcp.md`。
...
- DevFlow 的 Requirement / Master / Architect / Coding / Testing / Review / Delivery 都是可配置 Agent 角色：Requirement 负责需求 handoff，Master 负责 route / dispatch / rework checklist，Architect 承担 Plan 角色并输出技术方案、功能 todo list 和项目 todo list，Testing 在真实本地验证命令之后复核输出并生成最终 test report，Delivery 负责人审前交接；Review 会把 plan / todo / validation 作为核对依据。git、GitHub、checkpoint 和 merge 仍由 runtime 执行，职责边界见 `docs/devflow-agent-execution-policy.md`。
